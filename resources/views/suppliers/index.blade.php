<x-app-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Administra tu base de datos de proveedores</p>
        </div>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nombre, email o teléfono" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Ciudad</label>
                <input type="text" name="city" value="{{ request('city') }}" placeholder="Filtrar por ciudad" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
                    ...
                    @forelse($suppliers as $supplier)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $supplier->document_number ?? 'Sin documento' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($supplier->city)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('suppliers.destroy', $supplier) }}" class="inline" onsubmit="return confirm('¿Estás seguro de eliminar este proveedor?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Comienza agregando un nuevo proveedor.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
